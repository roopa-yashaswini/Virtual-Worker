...
<?php
    $account_type_err = $account_sub_type_err = '';
    $fname_err = $mname_err = $lname_err = $guardian_err = $relation_err = $gender_err = $dob_err = $nationality_err = $pan_exists_err = $pan_err  = $aadhar_err = "";
    $curr_address_err = $curr_city_err = $curr_pincode_err = $curr_district_err = $curr_state_err  = $curr_country_err = $permanent_address_check_err = $perm_address_err = $perm_district_err = $perm_city_err = $perm_pincode_err = $perm_state_err  = $perm_country_err = $permanent_address_check = $residence_type_err = $stay_period_err = $mobile_err = $alt_mobile_err = $email_err = $instant_alert_err = "";
    $occupation_type_err = $salaried_err = $company_name_err = $company_address_err = $gross_income_err = "";
    $customer_type_err = $customer_no_err = '';
    $photo_proof_err = $dob_proof_err = $identification_proof_err = $income_proof_err = "";
    $target_dir = $account_open_proofs_dir;
    $extensions_arr = array("jpg","jpeg","png");
    $fields_array = array(
        array("account_type", "account_type_err", "Select the account type"),
        array("account_sub_type", "account_sub_type_err", "Select the account sub type"),
        ...
        array("pan_exists", "pan_exists_err", "Select a value"),
        array('aadhar_no', 'aadhar_arr', 'Enter your aadhar number'),
    ...
    if($_SERVER['REQUEST_METHOD']=='POST'){
        ...
        $account_sub_type = isset($_POST['account_sub_type']) ? $_POST['account_sub_type'] : '';
        ...
        $pan_exists = isset($_POST['pan_exists']) ? $_POST['pan_exists'] : '';
        $pan = trim($_POST['pan']);
        ...
        $instant_alert = isset($_POST['instant_alert']) ? $_POST['instant_alert'] : '';
        ...
        $current_district = str_replace(" ", "", $current_district);
        $perm_district = str_replace(" ", "", $perm_district);

        if($account_type === 'Kids Advantage'){
            if(empty($guardian_name)){
                $guardian_err = 'Enter guardian name';
            }else{
                $guardian_err = '';
            }
            if(empty($relation)){
                $relation_err = 'Enter the relation';
            }else{
                $relation_err = '';
            }
        }else{
            if($occupation_type != 'Retired' && $occupation_type != 'Housewife' && $occupation_type != 'Student'){
        ...
        if(empty($dob)){
            ...
            if($account_type === 'Kids Advantage'){
                if($age >= 18){
                    $dob_err = "Age is not eligible for Kids Advantage account";
                }else{
                    $dob_err = '';
                }
            }else{
                if($age < 18){
                    $dob_err = "Age must be greater than or equal to 18";
                }else{
                    $dob_err = '';
                }
            }
        ...
        // if(!file_exists($_FILES['photo_proof']['tmp_name']) || !is_uploaded_file($_FILES['photo_proof']['tmp_name'])){
        ...
        if($account_type != 'Kids Advantage' && $occupation_type != 'Retired' && $occupation_type != 'Housewife' && $occupation_type != 'Student'){
        ...
        $errors_array = array($account_type_err,$account_sub_type_err,$fname_err,$mname_err,$lname_err,$guardian_err,$relation_err,$gender_err,$dob_err,$nationality_err,$pan_exists_err,$pan_err,$aadhar_err,$curr_address_err,$curr_city_err,$curr_pincode_err,$curr_district_err,$curr_state_err,$curr_country_err,$perm_address_err,$perm_city_err,$perm_pincode_err,$perm_district_err,$perm_state_err,$perm_country_err,$permanent_address_check_err,$residence_type_err,$stay_period_err,$mobile_err,$alt_mobile_err,$email_err,$instant_alert_err,$occupation_type_err,$salaried_err,$company_name_err,$company_address_err,$gross_income_err,$customer_no_err,$photo_proof_err,$dob_proof_err,$identification_proof_err,$income_proof_err);

        if(isempty($errors_array) == 0){
            if(move_uploaded_file($_FILES['photo_proof']['tmp_name'],$photo_proof_file) && move_uploaded_file($_FILES['dob_proof']['tmp_name'],$dob_proof_file) && move_uploaded_file($_FILES['identification_proof']['tmp_name'],$identification_proof_file)){
                if($account_type != 'Kids Advantage' && $occupation_type != 'Housewife' && $occupation_type != 'Retired' && $occupation_type != 'Student'){
                    if(move_uploaded_file($_FILES['income_proof']['tmp_name'],$income_proof_file)){
                        $sql = "INSERT INTO $table_account_open(account_type, account_sub_type,f_name,m_name,l_name,guardian_name,relation,gender,dob,nationality,pan_exists,pan_no,aadhar_no,current_address,current_pincode,current_district,current_city,current_state,current_country,permanent_address,permanent_pincode,permanent_district,permanent_city,permanent_state,permanent_country,residence_type,stay_period,mobile_no,alt_mobile_no,email,instant_alert,occupation_type,salaried_type
                        ,company_name,company_address,monthly_income, customer_type, customer_id, photo_proof,dob_proof,identification_proof,income_proof, token_number)
                        VALUES ('$account_type', '$account_sub_type', '$fname', '$mname', '$lname', '$guardian_name', '$relation', '$gender', '$dob', '$nationality', '$pan_exists', '$pan', '$aadhar_no', '$current_address', '$current_pincode', '$current_district', '$current_city', '$current_state', '$current_country', '$perm_address', '$perm_pincode', '$perm_district','$perm_city', '$perm_state', '$perm_country', '$residence_type', '$stay_period', '$mobile_no', '$alt_mobile_no', '$email', '$instant_alert', '$occupation_type', '$salaried_type', '$company_name', '$company_address', '$income', '$customer_type', '$customer_no', '$photo_proof_file', '$dob_proof_file', '$identification_proof_file', '$income_proof_file', '$token_number')";
...
<html>
    <head>
        ...
        <title>Account Opening Application</title>
