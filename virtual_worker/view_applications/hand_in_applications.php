            ...
            <div class="col-lg-9 my-lg-0 my-1">
                <div id="main-content" class="bg-white border">
                    <div class="row row-cols-1 row-cols-md-3 g-4">
                        <?php
                            $id = $_SESSION['user_id'];
                            $sql = "SELECT application_no, CONCAT_WS(' ', f_name, l_name) AS 'name', submit_date FROM $table_credit_card_applications WHERE handed_by = '$id' AND application_status = 'Handed In'"; //change to id
                            ...
                            $sql = "SELECT application_no, CONCAT_WS(' ', f_name, l_name) AS 'name', submit_date FROM $table_account_open WHERE handed_by = '$id' AND application_status = 'Handed In'"; //change to id
                            $result = mysqli_query($con, $sql);
                            if(mysqli_num_rows($result) > 0){
                                while($crow = mysqli_fetch_array($result, MYSQLI_ASSOC)){
                                    $crow['app_type'] = 'New Account';
                                    array_push($applications, $crow);
                                }
                            }
                            $sql = "SELECT application_no, CONCAT_WS(' ', f_name, l_name) AS 'name', submit_date FROM $table_loan_applications WHERE handed_by = '$id' AND application_status = 'Handed In'";    //change to id
                            $result = mysqli_query($con, $sql);
                            if(mysqli_num_rows($result) > 0){
                                while($crow = mysqli_fetch_array($result, MYSQLI_ASSOC)){
                                    $crow['app_type'] = 'Personal Loan';
                                    array_push($applications, $crow);
                                }
                            }
