                ...
                <div id="main-content" class="bg-white border">
                    <div class="row row-cols-1 row-cols-md-3 g-4">
                        <?php
                            ...
                            $sql = "SELECT application_no, CONCAT_WS(' ', f_name, l_name) AS 'name', submit_date FROM $table_account_open WHERE handed_by IS NULL AND current_district = '$district'";
                            $result = mysqli_query($con, $sql);
                            if(mysqli_num_rows($result) > 0){
                                while($crow = mysqli_fetch_array($result, MYSQLI_ASSOC)){
                            ...
                            $sql = "SELECT application_no, CONCAT_WS(' ', f_name, l_name) AS 'name', submit_date FROM $table_loan_applications WHERE handed_by IS NULL AND current_district = '$district'";
                            $result = mysqli_query($con, $sql);
                            if(mysqli_num_rows($result) > 0){
                                while($crow = mysqli_fetch_array($result, MYSQLI_ASSOC)){
